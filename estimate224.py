...
ITERATIONS = 252000 # NEEDS TO be a LCM of all the border thicknesses
...
def createWriteTable(list):
    ...
    while i <= borders:
        ...
        t.add_row([str(i), total, extra_pr_super, total_super, sgap[i-1],rgap[i-1],cgap[i-1],tgap[i-1], extra_points_over_calc_speed, gap_time,
                   runtime, round(perc, 2), round(ratio, 3), r])
...
if __name__ == "__main__":
    ...
    s = open("estimation224/special.txt", "w")
    s.close()
    readGaps()
    k = calcSpeed
    for i in sorted(datamap.keys()):
        k = calcSpeed
        f = open("estimation224/" + i + ".txt", "w")
        WIDTH = int(i)
        ...
        f.write(str(t) + "\n\n")
        if special:
            s = open("estimation224/special.txt", "a")
            s.write(str(info))
            s.write(str(t))
            s.close()
            # if(i == "500"):
