...
def addStuff(cores, dataSizes, tests, oneWays, secondWays, lat, latencies, moduleMatch, moduleFail):
    return
...
if __name__ == "__main__":
    ...
    i = 0
    while i < len(oneWays):
        comb.append((oneWays[i] + secondWays[i]) / 2)
        i= i + 1
    print(cores)
    print(dataSizes)
    print(comb)
    ...
    fileDir = "smallMessages_12_14.out"
    cores, dataSizes, tests, oneWays, secondWays,lat = readFile(dir + fileDir)
    latencies.append((cores, lat))
    ...
    print("nice!")
    plt.savefig("plots/hockney/small")
    #plt.show()
