...
def borderCalc(K):
    ...
    while j < K:
        # one extra row to calculate in top, and boto
        extra_points += HEIGHT * (K - j) * 2
        j+=1
    # calculate corners.
    corners = K * K * 4
    extra_points += corners
...
def createWriteTable(list):
    ...
    run_without_gap = rntime[0] - gap_time
    calcSpeed = ogCalc / run_without_gap
    ...
    t = PrettyTable(['Border Thickness', 'Stencils', 'Extra Stencils / Sstep', 'Ssteps', 'S gap / Sstep', 'R gap / Sstep', "C gap / Sstep", "T gap / Sstep",
                     'Extra stencils  / Calc speed','T gap * Supersteps', 'Runtime', '% increase',
                     '% more stencil points', "Physical runtime"])
    ...
    t.add_row(["1", ogCalc, None, float(ITERATIONS), sgap[0],rgap[0],cgap[0],tgap[0], None , gap_time, ogRuntime,
               None, round(float(0), 3), run_without_gap])
    ...
    while i <= borders:
        ...
        i += 1
    if percentages[0] < 99.5 and percentages[1] < 99.5:
        special = True
    if percentages[0] < 99:
        special = True
...
if __name__ == "__main__":
    ...
    s = open("estimation56C/special_cart.txt", "w")
    s.close()
    readGaps()
    k = calcSpeed
    for i in sorted(datamap.keys()):
        k = calcSpeed
        f = open("estimation56C/" + i + "_cart.txt", "w")
        WIDTH = int(i)
        calcSpeed = calcSpeedOG
        print(i + " " + str(k))
        info, t, special = createWriteTable(datamap[i])
        f.write(str(info))
        f.write(str(t) + "\n\n")
        if special:
            s = open("estimation56C/special_cart.txt", "a")
            s.write(str(info))
            s.write(str(t))
            s.close()
            # if(i == "500"):
